...
for flow in streamer:
    try:
        ...
        # Get duration in seconds for rate calculations
        duration_sec = max(getattr(flow, 'bidirectional_duration_ms', 0) / 1000.0, 0.001)
        ...
        for col in feature_names:
            ...
            # Convert durations to microseconds (CIC-IDS uses microseconds)
            if "Duration" in col or "IAT" in col:
                val = val * 1000

            # Compute RATES (not raw counts) for Bytes/s and Packets/s
            if col == 'Flow Bytes/s':
                val = getattr(flow, 'bidirectional_bytes', 0) / duration_sec
            elif col == 'Flow Packets/s':
                val = getattr(flow, 'bidirectional_packets', 0) / duration_sec
